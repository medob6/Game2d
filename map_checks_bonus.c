...
/*   Updated: 2025/01/14 10:44:24 by mbousset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*validate_map_items(t_game *game, char item, int *yx, int *items)
{
	if (item == 'P')
	{
		game->player.y = yx[0];
...
char	*check_map_c_p_e(t_game *game)
{
	...
	while (yx[0] < game->map.map_h)
	{
		...
		{
			error = validate_map_items(game, game->map.map[yx[0]][yx[1]], yx,items);
			if (error)
				return (error);
			yx[1]++;
		}
...
static int	validate_map_line(char **map, char *line, t_game *game, int i)
{
	...
	if (!line || (i == game->map.map_h - 1 && !check_border(line))|| !validate_line_length(line, width) || !validate_enclosed_line(line,width) || !check_line(line))
